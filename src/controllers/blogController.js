...
const Blogs = async function (req, res) {
  try {
    ...
    const authorReq = await authorModel.findById(authorId);
    if (authorReq) {
      ...
      res.status(400).send({
        status: false,
        msg: `${authorId} is not available, please enter valid authorId`,
      });
    }
  } catch (error) {
    ...
    if (Id) {
      ...
      if (data.isDeleted == false) {
        ...
        let data2 = await blogModel.findOneAndUpdate(
          { _id: Id },
          { title: value2, body: value1, tags: result1, subcategory: result2 },
          { new: true }
        );
        if (data.isPublished == false)
          data2 = await blogModel.findOneAndUpdate(
            { _id: Id },
        ...
        res
          .status(404)
          .send({ status: "false", msg: "data is already deleted" });
    } else res.status(404).send({ status: "false", msg: "id is not exist" });
  } catch (err) {
    ...
    if (blogs.length > 0) {
      for (let element of blogs) {
        if (
          ...
          element.authorId == req.decodeToken._id
        ) {
      ...
      if (m === 1) res.status(200).send({ status: true, msg: result });
      if (m === 0)
        res
          .status(400)
          .send({ status: false, msg: "this user can not delete the blogs" });
    } else
...
module.exports = { getBlogs, Blogs, deleting, updating, specificDeleting };
